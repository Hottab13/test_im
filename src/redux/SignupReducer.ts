...
export const actions = {
  onSignup: (isSignup: boolean) =>
    ({ type: "ON_SIGNUP", payload: { isSignup } } as const),
};
export const signupUser = (
  ...
  let respons = await userAPI.getSignupLogin(
    firstName,
    secondName,
    email,
    password
  ...
  if (respons.signup) {
    ...
    dispatch(
      stopSubmit("signup", { _error: respons.graphQLErrors[0].message })
    );
  }
};
