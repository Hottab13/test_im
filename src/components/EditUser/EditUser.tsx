...
import { matchPass, maxLengthCreator, requiredField } from '../utils/validators'
...
const UserEditForm:React.FC<InjectedFormProps<UserEditFormValueType,UserEditFormOwnProps>& UserEditFormOwnProps> =({handleSubmit, error, ...props }) =>{
...
<Row style={{height:"80px",marginLeft:"7px", marginRight:"7px"}}>
    ...
    <button disabled={!props.valid } className={cnu.button}>{editModeButton?"Сохранено":"Сохранить"}</button>
    </Col>
</Row>
<Row className={cnu.editForm} style={{marginLeft:"22px", marginRight:"22px"}} >
    <Col md={7}>
        <Row align="end" style={{height:"60px"}}>
            ...
            (userName.secondName,"firstName",[requiredField, maxLenght],InputControl)}
            </Col>
        </Row>
        <Row align="end" style={{height:"50px"}}>
            ...
            <Col >
            {createField<LoginFormValueTypeKey>
            (userName.firstName,"secondName",[requiredField, maxLenght],InputControl)}
            </Col>
        </Row>
        <Row align="end" style={{height:"50px"}}>
            <Col md={3} >
            ...
            <Col >
            {createField<LoginFormValueTypeKey>
            (userName.email ,"email",[requiredField, maxLenght],InputControl)}
            </Col>
        </Row>
        <Row align="end" style={{height:"50px"}}>
            <Col md={3} >
...
const onSubmit = (value:UserEditFormValueType )=>{
    dispatch(editUser (value.firstName,value.secondName,value.email,value.pass) )
}
if(!isAuth){
...
<Row className={cnu.menu}  >
    <Col md={2}  className={cnu.menuMod} >
        ...
        <Row style={{background: "#404064",height:"1064px"}} onClick ={deactivEditMode}></Row>
    </Col>
