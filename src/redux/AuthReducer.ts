...
export const actions = {
  setAuthUserData: (
    ...
    ({
      type: "SET_USER_DATA",
      payload: { userId, firstName, email, secondName, isAuth, token },
    } as const),
  setRenewalEditUser: (renewalEditUser: boolean) =>
    ({
      type: "SET_RENEWAL_EDIT_USER",
      renewalEditUser: { renewalEditUser },
    } as const),
};
...
export const loginUser = (
  ...
  if (respons.login) {
    ...
    dispatch(
      actions.setAuthUserData(id, firstName, email, secondName, true, token)
    );
  } else {
...
export const editUser = (
  ...
  let respons = await userAPI.getEditUserLogin(
    id,
    firstName,
    secondName,
    email,
  ...
  if (respons.editUser) {
    ...
    dispatch(
      stopSubmit("useredit", { _error: respons.graphQLErrors[0].message })
    );
  }
};
