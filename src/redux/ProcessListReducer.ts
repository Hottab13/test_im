...
const processListReducer=(state=initionalState,action:ActionType):InitionalStateType => {
    switch (action.type){
...
export const action={
    setUsers:(processList:Array<ProcessList>)=>({type:"SET_PROCESS_LIST", processList}as const),
    setCurrentPage:(setCurrentPage:number)=>({type:"SET_CURRENT_PAGE", setCurrentPage}as const),
...
export const getProcessListThunkCreator = ():ThunkActionType => 
