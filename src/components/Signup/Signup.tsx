    ...
    <form onSubmit={handleSubmit}>
      <Container fluid>
        <Row align="end" style={{ height: "60px", paddingLeft: "33px" }}>
          <Col>
            <div className={cl.signText}>Регистрация</div>
          </Col>
        </Row>
        ...
        <Row align="end" style={{ height: "60px" }}>
          <div className={cl.centrBut}>
            <button disabled={!props.valid} className={cl.button}>
              Применить и войти
            </button>
          </div>
        </Row>
        <Row align="end" style={{ height: "42px" }}>
          <Col className={cl.singPlank} offset={{ md: 1 }}>
            ...
            <Link to="/login" className={cl.sing}>
              Вход
            </Link>
          </Col>
        </Row>
...
export const Signup: React.FC = () => {
  ...
  const onSubmit = (value: LoginFormValueType) => {
    dispatch(
      signupUser(value.firstName, value.secondName, value.email, value.pass)
    );
  };
  if (isSignup) {
